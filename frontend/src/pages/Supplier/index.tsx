...
const SupplierList: React.FC = () => {
  ...
  const loadSuppliers = async () => {
    ...
    try {
      ...
      const response: any = await getSuppliers(params);
  ...
  return (
    <div>
      <div style={{ marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <Title level={2} style={{ margin: 0, fontWeight: 700, color: '#1e293b' }}>供应商管理</Title>
          <Text type="secondary" style={{ fontSize: 14, marginTop: 6, display: 'block' }}>管理供应商信息、联系人及资质</Text>
        </div>
        <Button 
          type="primary" 
          icon={<PlusOutlined />} 
        ...
        {currentSupplier && (
          <div style={{ padding: '16px 0' }}>
            <div style={{ marginBottom: 20 }}>
              ...
              <Tag color={currentSupplier.status === 'active' ? 'success' : 'error'}>
            ...
            <div style={{ background: '#f8fafc', borderRadius: 12, padding: 20 }}>
              <div style={{ marginBottom: 16 }}>
                ...
                <Text style={{ fontSize: 15, marginLeft: 8 }}>{currentSupplier.contact_name || '-'}</Text>
              </div>
              <div style={{ marginBottom: 16 }}>
                <Text type="secondary" style={{ fontSize: 13 }}>电话</Text>
                <Text style={{ fontSize: 15, marginLeft: 8 }}>{currentSupplier.phone || '-'}</Text>
              </div>
              <div style={{ marginBottom: 16 }}>
                <Text type="secondary" style={{ fontSize: 13 }}>邮箱</Text>
                <Text style={{ fontSize: 15, marginLeft: 8 }}>{currentSupplier.email || '-'}</Text>
              </div>
              <div style={{ marginBottom: 16 }}>
                <Text type="secondary" style={{ fontSize: 13 }}>税号</Text>
                <Text style={{ fontSize: 15, marginLeft: 8 }}>{currentSupplier.tax_id || '-'}</Text>
              </div>
              <div>
                <Text type="secondary" style={{ fontSize: 13 }}>地址</Text>
                <Text style={{ fontSize: 15, marginLeft: 8, display: 'block', marginTop: 8 }}>{currentSupplier.address || '-'}</Text>
              </div>
            </div>

            <div style={{ marginTop: 20, display: 'flex', justifyContent: 'flex-end', gap: 12 }}>
              <Button onClick={() => setDetailVisible(false)}>关闭</Button>
              <Button type="primary" onClick={() => { handleEdit(currentSupplier); setDetailVisible(false); }}>编辑</Button>
            </div>
          </div>
        )}
      </Modal>
